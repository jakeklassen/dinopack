...
const tests = [
  {
    ...
    bins: [new Bin("Le grande box", 100, 100, 300, 1500)],
    items: [new Item("Item 1", 150, 50, 50, 20)],
    expectation: function (packer: Packer) {
      return (
        packer.bins[0].items.length === 1 && packer.unfitItems.length === 0
      );
    },
  },
  {
    ...
    bins: [new Bin("Le grande box", 100, 100, 300, 1500)],
    items: [new Item("Item 1", 150, 50, 50, 20, [0, 3])],
    expectation: function (packer: Packer) {
      return (
        packer.bins[0].items.length === 1 && packer.unfitItems.length === 0
      );
  ...
  },
  {
    ...
    expectation: function (packer: Packer) {
      return (
        packer.bins[0].name === "1. Le petite box" &&
  ...
  },
  {
    ...
    expectation: function (packer: Packer) {
      return (
        packer.bins[0].items.length === 7 && packer.unfitItems.length === 0
      );
    },
  },
  {
    name: "Big item is packed first.",
    bins: [new Bin("Bin 1", 100, 100, 100, 1000)],
    items: [
    ...
    expectation: function (packer: Packer) {
      return (
        packer.bins[0].items.length === 1 && packer.unfitItems.length === 2
      );
    },
  },
  {
  ...
  },
  {
    ...
    expectation: function (packer: Packer) {
      return (
        packer.bins[0].items.length === 2 && packer.unfitItems.length === 0
      );
    },
  },
  {
    ...
    bins: [new Bin("Bin 1", 12, 12, 5.5, 70)],
    items: [
      new Item("Item 1", 12, 12, 0.005, 0.0375),
      new Item("Item 2", 12, 12, 0.005, 0.0375),
    ],
    expectation: function (packer: Packer) {
      return (
        packer.bins[0].items.length === 2 && packer.unfitItems.length === 0
...
Deno.test("Packer", async (t) => {
  for (const { name, bins, items, expectation } of tests) {
    await t.step(name, () => {
      const packer = new Packer(bins, items);
      packer.pack();

      assertEquals(expectation(packer), true);
    });
  }
});
